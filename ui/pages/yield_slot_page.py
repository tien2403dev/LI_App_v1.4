...
class YieldSlotPage(QWidget):
    ...
    def load_result(self, result, key):
        if not result.eqp:
            self.clear_result('Chọn một EQP rồi bấm SEARCH.')
    ...
    def set_scrap_codes(self, codes, redraw=False):
        """Scrap chỉ thay biểu đồ CUM theo Model; không tác động bảng PRIME Slot."""
        self._selected_scrap_codes = tuple(codes)
        if redraw:
            self.model_summary.update_codes(codes)
